RSpec.describe QPointF do
  it "#initialize" do
    o = described_class.new(1.2, 3.4)
    expect(o.x).to eq(1.2)
    expect(o.y).to eq(3.4)
  end

  it "#==" do
    expect(described_class.new(1.0, 2.0)).to eq(described_class.new(1.0, 2.0))
    expect(described_class.new(1.0, 2.0)).not_to eq(described_class.new(1.0, 2.1))

    expect(described_class.new(1.0, 2.0)).to eq(QPoint.new(1, 2))
    expect(described_class.new(1.0, 2.0)).not_to eq(QPoint.new(1, 3))

    expect(described_class.new(1.0, 2.0)).not_to eq(1.0)
    expect(described_class.new(1.0, 2.0)).not_to eq(nil)
  end

  it "#*" do
    o = described_class.new(1, 2) * 4
    expect(o).to be_a(described_class)
    expect(o.x).to eq(4)
    expect(o.y).to eq(8)

    o = described_class.new(1, 2) * 3.3
    expect(o).to be_a(described_class)
    expect(o.x).to eq(3.3)
    expect(o.y).to eq(6.6)
  end

  it "#+" do
    o = described_class.new(1, 2) + described_class.new(4, 7)
    expect(o).to be_a(described_class)
    expect(o.x).to eq(5)
    expect(o.y).to eq(9)
  end

  it "#-" do
    o = described_class.new(1, 2) - described_class.new(4, 7)
    expect(o).to be_a(described_class)
    expect(o.x).to eq(-3)
    expect(o.y).to eq(-5)
  end

  it "#/" do
    o = described_class.new(1, 2) / 3
    expect(o).to be_a(described_class)
    expect(o.x).to eq(0.3333333333333333)
    expect(o.y).to eq(0.6666666666666666)
  end

  it "#+@" do
    o1 = described_class.new(1, 2)
    o2 = + o1
    expect(o2).to be_a(described_class)
    expect(o1).to eq(o2)
    expect(o1.object_id).not_to eq(o2.object_id)
  end

  it "#-@" do
    o = - described_class.new(1, 2)
    expect(o).to be_a(described_class)
    expect(o.x).to eq(-1)
    expect(o.y).to eq(-2)
  end
end
